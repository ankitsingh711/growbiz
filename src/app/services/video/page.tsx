...
export default function VideoServicePage() {
    ...
    const relatedStudies = caseStudies.filter((study) => service.relatedCaseStudies?.includes(study.slug))
    ...
    return (
        <main className="min-h-screen bg-white">
                ...
                <div className="container relative z-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        ...
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
                            <img src="https://images.unsplash.com/photo-1492619375914-88005aa9e8fb?auto=format&fit=crop&q=80&w=800" alt="Video Marketing" className="rounded-2xl shadow-2xl" />
                        </motion.div>
                    </div>
                </div>
            </section>
            ...
            {relatedStudies.length > 0 && (
                <section className="py-24 container">
                    <div className="text-center mb-16">
                        <h2 className="text-3xl md:text-5xl font-bold font-heading">Video <span className="text-brand-yellow">Success Stories</span></h2>
                    </div>
                    <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                        {relatedStudies.map((study) => (
                            <Link key={study.slug} href={`/case-studies/${study.slug}`}>
                                <div className="group bg-white rounded-2xl overflow-hidden border hover:shadow-xl transition-all">
                                    <div className="relative h-48">
                                        <img src={study.thumbnail} alt={study.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                                    </div>
                                    <div className="p-6">
                                        <h3 className="text-xl font-bold mb-3 group-hover:text-brand-yellow transition-colors">{study.title}</h3>
                                        <p className="text-gray-600 mb-4">{study.summary}</p>
                                        <div className="flex items-center text-brand-yellow font-medium">Read Case Study <ArrowRight size={16} className="ml-2" /></div>
                                    </div>
                                </div>
                            </Link>
                        ))}
            ...
            <CTA variant="dark" title="Create Engaging Video Content" buttonText="Start Video Project" />
            <Footer />
        </main>
    )
}
